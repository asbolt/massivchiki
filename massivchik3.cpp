...
int createMatrixes (int **matrix, Matrix2 *matrix2, int *sizeMatrix, int *rows)
{
    ...
    *matrix = (int *)calloc (*sizeMatrix, sizeof(int));
    ...
    matrix2->adress = (int *)calloc (*rows, sizeof(int));
...
int scanAmountSumbols (Matrix2 *matrix2, int rows, int sizeMatrix)
{
    ...
    for (int rowIndex = 0; rowIndex < rows-1; rowIndex++)
    {
        ...
        else if (symbolsAmount == sizeMatrix && rowIndex != rowIndex - 1)
        {
            printf ("А нахер тебе остальные ряды?\n");
            return 1;
        }

        matrix2->amountSumbols[rowIndex] = symbols;

        matrix2->adress [rowIndex+1] = matrix2->adress [rowIndex] + symbols;
    }
...
int scanSymbols (int rows, Matrix2 matrix2, int **matrix)
{
    ...
    for (int rowIndex = 0; rowIndex < rows; rowIndex++)
    {
        ...
        for (int symbols = 0; symbols < matrix2.amountSumbols[rowIndex]; symbols++)
        {
...
int printMatrix (int rows, Matrix2 matrix2)
{
    for (int rowIndex = 0; rowIndex < rows; rowIndex++)
    {
        for (int symbols = 0; symbols < matrix2.amountSumbols[rowIndex]; symbols++)
        {
            printf ("%d", *matrix2.adress + symbols);
        }
